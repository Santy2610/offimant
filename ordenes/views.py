...
def ordenes(request):
    ...
    dato = "SED"
    return render(request, "listadoordenes.html", {"ordensw": ordenlist, "paginador": paginador, "listpsw": ordenlist, "datosw": dato, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def ordenesnew(request, vista, dato, page):
    if vista == 'index':
        dept = orden.objects.values('departamento').order_by(
            'departamento').annotate(contador=Count('departamento'))
        formorden = fomularioorden()
        return render(request, "adicionarordenes.html", {"form": formorden, "vistasw": vista, "datosw": dato, "deptsw": dept, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
    else:
        ...
        clog = orden.objects.values('departamento').order_by(
            'departamento').annotate(contador=Count('departamento'))
        return render(request, "adicionarordenes.html", {"form": formorden, "resultsw": result, "fallasw": falla, "vistasw": vista, "datosw": dato, "pagesw": page, "clogsw": clog, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
