...
def listadotiempo(request):
    ...
    Grafth = tiempo.objects.values('area').order_by(
        'area').annotate(canti=Sum('dias'))
    total = 0
    for item in Grafth:
    ...
    return render(request, "listadotiempo.html", {"listFor": listF, "listTsw": listT, "datosTP": datosTP, "paginador": paginador, "listpsw": listT, "Grafthsw": Grafth, "totalSW": total, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def tiempoadd(request):
    ...
    listT = tiempo.objects.create(area=area, equipo=equipo, fechaI=fechaI,
                                  fechaF=fechaF, causa=causa, observacion=observacion, dias=time)
    listT.save()
...
def imptiemp(request):
    ...
    return render(request, "implistadotiempo.html", {"listTsw": listT, "datosTP": datosTP, "Grafthsw": Grafth, "totalSW": total, 'tiempopSW': tiempoP()})
