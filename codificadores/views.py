...
def codunidad(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formUnidad = formulariounidadm(
            initial={'Unidad': unidg.unid, 'Descripcion': unidg.descripcion})
    return render(request, "unidadindex.html", {"form": formUnidad, "unisw": unidlist, "vistasw": vista, "dato": dato, "paginador": paginador, "listpsw": unidlist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codalmacen(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formAlmacen = formularioalmacen(
            initial={'Codigo': almaceng.codigo, 'Descripcion': almaceng.descripcion})
    return render(request, "almacenesindex.html", {"form": formAlmacen, "Almasw": Almalist, "vistasw": vista, "dato": dato, "paginador": paginador, "listpsw": Almalist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codcosto(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formcosto = formulariocosto(initial={
                                    'Codigo': costong.codigo, 'Descripcion': costong.descripcion, 'Produccion': costong.prod})
    return render(request, "costoindex.html", {"form": formcosto, "costosw": costolist, "vistasw": vista, "dato": dato, "paginador": paginador, "listpsw": costolist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codcostoadd(request):
    ...
    costolist = centrocosto.objects.create(
        codigo=codigo, descripcion=descripcion, prod=prod)
...
def codareas(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formareas = formularioareas(
            initial={'Codigo': areasdg.codigo, 'Descripcion': areasdg.descripcion})
    return render(request, "areasindex.html", {"form": formareas, "areassw": areaslist, "dato": dato, "vistasw": vista, "paginador": paginador, "listpsw": areaslist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codequipos(request, dato):
    areaslist = areas.objects.get(pk=dato)
    page = request.GET.get('page', 1)
    equiposlist = equipos.objects.filter(
        masterArea=areaslist).order_by('codigo')
    paginador = Paginator(equiposlist, 10)
    equiposlist = paginador.page(page)
    equiposbot = equipos.objects.filter(
        masterArea=areaslist).order_by('codigo')
    formequipos = formularioequipos()
    return render(request, "equiposindex.html", {"form": formequipos, "areassw": areaslist, "equipossw": equiposlist, "equiposbot": equiposbot, "paginador": paginador, "listpsw": equiposlist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codequiposadd(request, dato):
    ...
    equiposlist = equipos.objects.create(
        masterArea=arealist, codigo=codigo, descripcion=descripcion)
...
def codequiposedit(request, dato, area):
    ...
    formequipos = formularioequipos(
        initial={'Codigo': equiposdg.codigo, 'Descripcion': equiposdg.descripcion})
    return render(request, "equiposedit.html", {"form": formequipos, "equipossw": equiposlist, "equiposbot": equiposbot, "areassw": areaslist, "area": area, "dato": dato, "paginador": paginador, "listpsw": equiposlist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codcausas(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formcausas = formulariocausas(
            initial={'Codigo': causasdg.codigo, 'Descripcion': causasdg.descripcion})
    return render(request, "causasindex.html", {"form": formcausas, "causassw": causalist, "vistasw": vista, "dato": dato, "paginador": paginador, "listpsw": causalist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codtraba(request, vista, dato):
    ...
    if vista == 'index':
        ...
        formtrab = formulariotrab(initial={
                                  'codigo': trabdg.codigo, 'nombre': trabdg.nombre, 'Solic': trabdg.solic, 'Person': trabdg.person})
    return render(request, "trabajadoresindex.html", {"form": formtrab, "trabsw": trablist, "vistasw": vista, "dato": dato, "paginador": paginador, "listpsw": trablist, "contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
...
def codtrabaadd(request):
    ...
    trablist = trabajadores.objects.create(
        codigo=codigo, nombre=nombre, solic=solic, person=person)
...
def codparada(request):
    return render(request, "paradasindex.html", {"contadorSW": barracont(), "ordenmSW": tareaM(), 'tiempopSW': tiempoP()})
